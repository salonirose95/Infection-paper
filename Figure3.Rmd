...
#reading file
df <- read_excel("Figure3-data.xlsx", sheet="Sheet1")
...
numlist3 <-c(0,500,1000,1500,2000)
```
...
p1 <- ggplot(data=df, aes(x = factor(Treatment, level = list3), y=CI)) +stat_boxplot(geom = 'errorbar',width = 0.15)+ geom_boxplot(width=0.45,outlier.size = -1, fill=list2)+ ylab("Courtship Index")+ xlab("Female Treatment") +scale_y_continuous(expand = c(0, 0),breaks=numlist)+ theme1+ coord_cartesian(clip="off", ylim=c(0, 100))+ facet_wrap(~ Group, scale="free")

p11 <-ggplot(data=df, aes(x = factor(Treatment, level = list3), y=CoL)) + stat_boxplot(geom = 'errorbar',width = 0.15)+ geom_boxplot(width=0.5,outlier.size = -1,fill= list2)+
  ylab("Copulation Latency(s)")+ 
  xlab("Female Treatment") + theme1 +scale_y_continuous(expand = c(0, 0),breaks=numlist3)+coord_cartesian(clip="off",ylim= c(0,1500))+facet_wrap(~ Group, scale="free")
...
grid.arrange(grobs=list(p11,p2),ncol=1)
````
